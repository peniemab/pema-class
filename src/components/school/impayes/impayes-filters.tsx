...
export function ImpayesFilters({ classes, fees, filters }: Props) {
  const router = useRouter();
  const searchParams = useSearchParams();

  const [search, setSearch] = useState(filters.search ?? '');
  const [classId, setClassId] = useState(filters.classId ?? '');
  const [feeId, setFeeId] = useState(filters.feeId ?? '');
  ...
  useEffect(() => {
    ...
    setFeeId(filters.feeId ?? '');
  }, [filters.search, filters.classId, filters.feeId]);
  ...
  const apply = useCallback(
    (next: Record<string, string | undefined>) => {
      ...
      params.delete('page');
      router.push(`/school/impayes?${params.toString()}`);
    },
    [router, searchParams],
  );
  ...
  function clearFilters() {
    router.push('/school/impayes');
  }

  const hasFilters =
    Boolean(filters.search) ||
    Boolean(filters.classId) ||
    Boolean(filters.feeId);
    ...
    >
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="impayes-classe">Classe</Label>
          <select
            id="impayes-classe"
            name="classe"
            value={classId}
        ...
        <div className="space-y-1.5">
          <Label htmlFor="impayes-frais">Type de frais</Label>
          <select
            id="impayes-frais"
            name="frais"
            value={feeId}
