...
import { enrollStudentAction } from '@/lib/school/students-actions';
...
export function EnrollStudentForm({ activeYearName, classes }: Props) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="rounded-lg border bg-muted/20 px-3 py-2.5 text-sm">
              ...
              {assignedSection ? (
                <p className="mt-1 text-muted-foreground">
                  {classDisplayLabel(assignedSection.level, assignedSection.name)}{' '}
                  ({assignedSection.current_count}/{assignedSection.max_capacity}) — première
                  section disponible (A, puis B…).
                </p>
              ) : (
                <p className="mt-1 text-destructive">
                  ...
                  {sectionsForLevel
                    .map((c) =>
                      sectionAvailabilityLabel(c.name, c.current_count, c.max_capacity),
                    )
                    .join(', ')}
                </p>
              )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Saisissez le nom et le post-nom dans le même champ, en majuscules.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2 sm:col-span-2">
            <label className="flex items-center gap-2 text-sm">
              ...
              />
              Générer le matricule automatiquement (unique à cet établissement)
            </label>
            {!autoMatricule ? (
              <Input
                value={matricule}
